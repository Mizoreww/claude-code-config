...
`settings.json` 默认开启了一组高性能配置。未知键在旧版 Claude Code 上会被静默忽略，因此除 `auto` 模式外都不需要版本门控（auto 由安装器处理降级）。

| 配置项 | 默认值 | 作用 |
|--------|--------|------|
| `permissions.defaultMode` | `"auto"` | Claude 自动批准安全操作、拦截高风险操作。需要 Claude Code ≥ 2.1.80；安装器在更旧的 CLI 上自动降级为 `bypassPermissions`。 |
| `effortLevel` | `"max"` | 默认把 `/effort` 固定在最高推理档。不识别 `max` 的旧 CLI 会自动回退到其支持的最高档。 |
| `betas` | `["extended-cache-ttl-2025-04-11"]` | 启用 1 小时提示缓存 TTL beta，替代默认的 5 分钟 TTL。 |
| `env.CLAUDE_CODE_NO_FLICKER` | `"1"` | 切换到全屏渲染模式（等价于 `/tui fullscreen`）。 |
| `env.CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING` | `"1"` | 把思考预算固定到 `MAX_THINKING_TOKENS`，关闭按轮自适应。Opus 4.7 不受影响（始终自适应）。 |

如需其他默认，直接编辑 `settings.json` —— 例如把 `defaultMode` 改为 `"default"`、`"acceptEdits"` 或 `"bypassPermissions"`；或删除 `effortLevel` 改由会话内 `/effort` 手动调节。
